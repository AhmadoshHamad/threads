      ...
      //  sleep(1);
        ...
        if(numOfAddEmp != maxNumOfEmp){ // max number  of emp can i add is 2 
            if (p - i == THRESHOLD){//THRESHOLD
                ...
                if (kill(getppid(), SIGUSR1) == 0) { // Use 'sig' instead of 'signal'
                    printf("Thread: Signal sent successfully\n");
                } else {
                    perror("Thread: Error sending signal");
                }  
                // Open the FIFO for writing
                int fd = open(fifo_lead, O_WRONLY);
                if (fd == -1) {
...
//    pthread_create(&check_thread, NULL, checking, NULL);
    ...
    pthread_t thread;
    pthread_create(&thread, NULL, checking, NULL);
